...
export default class ImportService {
    ...
    async processFile(
        ...
        try {
            ...
            const sheetName = await this.fileService.getSheetName(
                workbook,
                sheetOption || this.component.getReadSheet(),
                this.i18nResource
            );
            ...
            const extractResult = this.component.getStandalone() && this.component.getReadAllSheets()
                ? this.dataExtractorService.extractAllSheetsData(workbook)
                : this.dataExtractorService.extractSheetData(
                    workbook,
                    sheetName,
                    coordinates || this.component.getReadSheetCoordinates()
                );
    ...
    async validateData(
        ...
        columnNames: string[]
    ): Promise<{ isValid: boolean, messages: any[] }> {
    ...
    ): Promise<{ isValid: boolean, messages: any[], processedData: any }> {
        try {
            // Validate data
            const validation = await this.validateData(
                processedData.spreadsheetSheetsData,
    ...
    async processAndValidate(
        ...
        try {
            ...
            const processedData = await this.processFileData(
                fileOrWorkbook,
            ...
            if (validate) {
                const validationResult = await this.validateProcessedData(
                    processedData,
                    showMessages
                );
    ...
    async runImportPipeline(
        ...
        return this.processValidateAndUpload(
            fileOrWorkbook,
            sheetOptionOrSheetName,
            coordinates,
            pipelineOptions,
     ...
     */
    async getWorkbookInfo(file: File | Blob, sheetOption?: string | number): Promise<{
    ...
    validateHeaders(columnNames: string[]): { isValid: boolean, issues: string[] } {
        return this.validationService.validateHeaders(
            columnNames,
            this.spreadsheetUploadController.typeLabelList
        );
    }
