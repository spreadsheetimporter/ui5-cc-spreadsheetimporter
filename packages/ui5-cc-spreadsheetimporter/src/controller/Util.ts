...
export default class Util extends ManagedObject {
  ...
  static getValueFromRow(row: RowData, label: string, type: string, fieldMatchType: FieldMatchType): ValueData {
    ...
    if (fieldMatchType === 'labelTypeBrackets') {
      try {
        value = Object.entries(row).find(([key]) => Util.columnMatchesType(key, type))[1] as ValueData;
      } catch (error) {
  ...
  /**
   * Case-insensitive test whether a spreadsheet column header matches an OData property for
   * fieldMatchType 'labelTypeBrackets' (header "<label> [<Property>]"). The bracketed technical
   * token is compared without regard to case, so a template generated server-side by the ABAP
   * backend - which on S/4HANA 2023 can only emit the element name in UPPER CASE
   * ("ORDERID [ORDERID]") - re-imports identically to one generated by this component
   * ("OrderId [OrderId]"). Plain field names round-trip on the server the same way.
   */
  static columnMatchesType(columnKey: string, type: string): boolean {
    if (!columnKey || typeof columnKey !== 'string') {
      return false;
    }
    return columnKey.toUpperCase().includes(`[${type}]`.toUpperCase());
  }
