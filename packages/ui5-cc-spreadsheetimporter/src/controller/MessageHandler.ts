...
export default class MessageHandler extends ManagedObject {
  ...
  addArrayToMessages(messages: Messages[]) {
    Log.debug('addArrayToMessages', undefined, 'SpreadsheetUpload: MessageHandler', () =>
      this.spreadsheetUploadController.component.logger.returnObject(messages)
    );
    this.messages = this.messages.concat(messages);
  }

  addMessageToMessages(message: Messages) {
    Log.debug('addMessageToMessages', undefined, 'SpreadsheetUpload: MessageHandler', () =>
      this.spreadsheetUploadController.component.logger.returnObject(message)
    );
  ...
  checkMandatoryColumns(
    data: PayloadArray,
    columnNames: string[],
    mandatoryFieldsUser: string[],
    mandatoryFieldsMetadata: string[],
  ...
  checkMandatoryFields(data: ArrayData, mandatoryFields: string[], typeLabelList: ListObject) {
    ...
    for (const mandatoryField of mandatoryFields) {
      ...
      for (const [index, row] of data.entries()) {
        const value = Util.getValueFromRow(
          row,
          fieldLabel,
          mandatoryField,
          this.spreadsheetUploadController.component.getFieldMatchType() as FieldMatchType
        );
        const errorMessage = {
      ...
      for (const [key, valueTypeLabelList] of typeLabelList) {
        if (valueTypeLabelList.maxLength) {
          ...
          if (value && value.rawValue && value.rawValue.length > valueTypeLabelList.maxLength) {
            ...
            const errorMessage = {
              title: this.spreadsheetUploadController.util.geti18nText('spreadsheetimporter.maxLengthExceeded', [
                valueTypeLabelList.maxLength,
                valueTypeLabelList.label
              ]),
  ...
  checkEmptyHeaders(data: ArrayData, columnNames: string[]) {
    ...
    const emptyColumns = [...emptyColumnsFromHeaders, ...Array.from(emptyColumnsFromData)].filter(
  ...
  groupMessages(messages: Messages[]): GroupedMessage[] {
    ...
    const messageGroups = parsingMessages.reduce<{ [key: string]: MessagesDetails[] }>((groups, message) => {
      ...
      if (message.maxLength && message.excededLength) {
        messageText = this.spreadsheetUploadController.util.geti18nText('spreadsheetimporter.maxLengthExceededWithLength', [
          message.maxLength,
          message.excededLength,
          message.rawValue
        ]);
      } else if (message.rawValue && message.formattedValue) {
        messageText = this.spreadsheetUploadController.util.geti18nText('spreadsheetimporter.errorInRowWithValueFormatted', [
          message.row,
