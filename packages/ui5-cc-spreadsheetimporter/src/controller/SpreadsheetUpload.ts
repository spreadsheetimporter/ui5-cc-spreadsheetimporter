...
export default class SpreadsheetUpload extends ManagedObject {
  ...
  constructor(component: Component, componentI18n: ResourceModel) {
    ...
    if (this.component.getI18nModel()) {
      try {
        ...
        Log.error('Error enhancing i18n model', error as Error, 'SpreadsheetUpload: SpreadsheetUpload', () =>
      ...
      .then(
        function (versionInfo: any) {
          ...
          Log.debug('constructor', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
            this.component.logger.returnObject({ ui5version: version, isOpenUI5: this.isOpenUI5 })
          );
        }.bind(this)
      );
  ...
  async setContext() {
    ...
    if (this.component.getBindingCustom()) {
      ...
      Log.debug('tableObject', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
        this.component.logger.returnObject({ tableObject: this.tableObject })
      );
      if (!this.tableObject) {
        throw new Error('No table object found');
      }
      this.component.setTableId(this.tableObject.getId());
      Log.debug('table Id', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
        this.component.logger.returnObject({ tableID: this.tableObject.getId() })
      );
    ...
    Log.debug('odataType', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
      this.component.logger.returnObject({ odataType: this._odataType })
    );
    this.odataKeyList = await this.odataHandler.getKeyList(this._odataType, this.binding);
    Log.debug('odataKeyList', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
      this.component.logger.returnObject({ odataKeyList: this.odataKeyList })
    );
    this.typeLabelList = await this.odataHandler.getLabelList(
      this.component.getColumns(),
      this._odataType,
    ...
    Log.debug('typeLabelList', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
      this.component.logger.returnObject({ typeLabelList: this.typeLabelList })
    );

    if (this.isODataV4) {
      const { mainEntity, expands } = this.odataHandler.getODataEntitiesRecursive(this.getOdataType(), 99);
      Log.debug('mainEntity', undefined, 'SpreadsheetUpload: SpreadsheetUpload', () =>
        this.component.logger.returnObject({ mainEntity: mainEntity })
      );
  ...
  setComponentOptions(options: ComponentData) {
    ...
    if (options.hasOwnProperty('deepDownloadConfig')) {
      this.component.setDeepDownloadConfig(
        Util.mergeDeepDownloadConfig(this.component.getDeepDownloadConfig() as DeepDownloadConfig, options.deepDownloadConfig)
      );
    }
