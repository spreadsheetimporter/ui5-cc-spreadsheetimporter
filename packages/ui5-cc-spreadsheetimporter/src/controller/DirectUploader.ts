  ...
  async uploadFile(fileContent: ArrayBuffer, fileName: string, entityNameBinding: string): Promise<any> {
    ...
    if (this.config.uploadUrl) {
      ...
      if (!this.config.entityName && !entityNameBinding) {
        ...
        Log.error('No entity name provided for direct upload', error, 'DirectUploader', () =>
          this.component.logger.returnObject({ config: this.config })
        );
        throw error;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          Log.error('Direct upload failed', error, 'DirectUploader', () =>
            this.component.logger.returnObject({
              status: xhr.status,
      ...
      xhr.onerror = () => {
        ...
        Log.error('Network error during direct upload', error, 'DirectUploader', () =>
          this.component.logger.returnObject({
            url: xhr.responseURL,
      ...
      xhr.ontimeout = () => {
        ...
        Log.error('Timeout during direct upload', error, 'DirectUploader', () =>
          this.component.logger.returnObject({
            url: xhr.responseURL,
            timeout: xhr.timeout
          })
