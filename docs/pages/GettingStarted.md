...
2\. **Add `resourceRoots` to your `manifest.json` under `sap.ui5`:**
   ...
   ```json
   "resourceRoots": {
     "cc.spreadsheetimporter.v1_2_1": "./thirdparty/customcontrol/spreadsheetimporter/v1_2_1"
   }
   ```
...
4\. **Add `componentUsages` to your `manifest.json` under `sap.ui5`:**
   ...
   "componentUsages": {
     "spreadsheetImporter": {
       "name": "cc.spreadsheetimporter.v1_2_1"
     }
   }
   ```
...
5\. **Optional: Handle the "component does not exist" error**
   ...
   ```json
   "sap.app": {
     "embeds": ["thirdparty/customcontrol/spreadsheetimporter/v1_2_1"]
   }
   ```
...
When deploying the app to your ABAP system, you might encounter an error like `SAPUI5 library/component cc.spreadsheetimporter.v1_2_1 used in application Z*** does not exist`. The application is deployed, but the service returns an error.
  ...
  customTasks:
    - name: ui5-task-zipper
      ...
      configuration:
        ...
        includeDependencies:
        - ui5-cc-spreadsheetimporter-v1-2-1
```
...
`"Service name 'spreadsheetimporter_v1_2_1' and public setting 'true' in embedded manifest.json have to be equal to service name 'xxxxxxx' and public setting 'true' of root manifest.json"`
...
This task will update the Spreadsheet Importer manifest with the app's service name at this path: `dist/thirdparty/customcontrol/spreadsheetimporter/v1_2_1/manifest.json`.
