...
**CI** (`.github/workflows/wdi5-test.yml`): matrix of 6 scenarios × 3 UI5 versions (`136`/`120`/`71`; trimmed from 6 — newest default, the 1.120 FE-metadata divergence, and the 1.71 floor). It removes `cds-plugin-ui5`, installs, `copyTestApps`, builds, starts the CAP server + the app (`start:silent`, which uses `ui5-test.yaml` and reaches the component through the CAP server), waits for ports `4004` + the app port, then runs wdi5 headless and uploads the timeline report artifact.
...
npm run lint:ui5                 # ui5lint (component workspace)
