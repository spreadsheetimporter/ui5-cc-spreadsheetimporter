	...
	it("should verify uploaded order items for coordinates", async () => {
		...
		try {
			...
			if (response.ok) {
				...
				const order5WithItems = data.value.find((order) => order.OrderNo === "5" && order.Items && order.Items.length > 0);
				const order6WithItems = data.value.find((order) => order.OrderNo === "6" && order.Items && order.Items.length > 0);
